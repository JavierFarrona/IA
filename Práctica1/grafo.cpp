...
void Grafo::loadGraphFromFile(const string &filename) {
  ...
  matriz_.resize(
      num,
      vector<double>(num, -1.00));  // Redimensionar la matriz de adyacencia
...
void Grafo::BFS(int startVertex, int endVertex, ostream &out) {
  ...
  toExplore.push_back(
      startVertex);  // Agregar el nodo de inicio a la lista de exploración
  visited[startVertex] = true;  // Marcar el nodo de inicio como visitado
  int currentIndex = 0;
  bool found = false;
  int iteration = 1;
  vector<int> newGenerated;  // Para almacenar los nuevos nodos generados en
                             // esta iteración
  ...
  while (currentIndex < toExplore.size()) {
    ...
    inspected.push_back(
        current);  // Agregar el nodo actual a la lista de inspeccionados
    out << "--------------------------------" << endl;
    out << "Iteración " << iteration++ << endl;
    out << "Nodos generados: ";
    ...
    for (int i = 0; i < nodos_; ++i) {
      if (matriz_[current][i] != INF && !visited[i]) {
        ...
        if (i == endVertex) {
          found = true;  // Si encontramos el nodo de destino, marcar como
                         // encontrado
        }
      }
    }
    ...
    currentIndex++;
    if (found)
      break;  // Si encontramos el vértice de destino, salimos del bucle
  }
  ...
  if (found) {
    vector<int> path;
    double totalCost =
        0.0;  // Variable para almacenar el coste total del camino
    for (int v = endVertex; v != -1; v = parent[v]) {
      path.push_back(v + 1);  // Convertimos a 1-based index
      if (parent[v] != -1) {
        totalCost +=
            matriz_[parent[v]]
                   [v];  // Sumar el coste de la arista entre parent[v] y v
      }
    }
    reverse(path.begin(),
            path.end());  // Invertir el camino para obtener el orden correcto
    ...
    out << "No se encontró un camino desde " << startVertex + 1 << " hasta "
        << endVertex + 1 << endl;
  }
}
/*
void Grafo::BFS_MODI(int startVertex, int endVertex, ostream &out) {
    vector<bool> visited(nodos_, false);  // Para rastrear nodos visitados
    vector<int> parent(nodos_, -1);       // Para reconstruir el camino
    vector<pair<double, int>> toExplore;  // Nodos por explorar, {coste
acumulado, nodo} vector<int> inspected;                // Nodos inspeccionados
    int numEdges = 0;                     // Contador de aristas
    ...
    visited[startVertex] = true;  // Marcar el nodo de inicio como visitado
    bool found = false; // Variable para indicar si se encontró el nodo de
destino int iteration = 1; // Contador de iteraciones double Cost = 0.0;  //
Variable para almacenar el coste total del camino
...
esta iteración
    ...
    while (!toExplore.empty()) {
        sort(toExplore.begin(), toExplore.end());  // Ordenar los nodos por
coste acumulado

        // Elección aleatoria entre el mejor o peor nodo
        int current;
        if (rand() % 2 == 0) {
            current = toExplore.front().second;  // Elegir el mejor (menor
coste) Cost = toExplore.front().first;  // Actualizar el coste acumulado
        ...
        inspected.push_back(current);  // Agregar el nodo actual a la lista de
inspeccionados out << "--------------------------------" << endl; out <<
"Iteración " << iteration++ << endl;
...
encontrado
        ...
        if (found) break;  // Si encontramos el vértice de destino, salimos del
bucle
    }
    ...
    if (found) {
        vector<int> path;
        double totalCost = 0.0;  // Variable para almacenar el coste total del
camino for (int v = endVertex; v != -1; v = parent[v]) { path.push_back(v + 1);
// Convertimos a índice 1-based if (parent[v] != -1) { totalCost +=
matriz_[parent[v]][v];  // Sumar el coste de la arista
            }
        }
        reverse(path.begin(), path.end());  // Invertir el camino para obtener
el orden correcto
...
void Grafo::DFS(int startVertex, int endVertex, ostream &out) {
  ...
  while (currentIndex >= 0) {
    ...
    inspected.push_back(
        current);  // Agregar el nodo actual a la lista de inspeccionados
    toExplore
        .pop_back();  // Lo eliminamos del vector para simular el backtracking
    out << "--------------------------------" << endl;
    out << "Iteración " << iteration++ << endl;
    out << "Nodos generados: ";
    ...
    for (int i = 0; i < nodos_; ++i) {
      if (matriz_[current][i] != INF && !visited[i]) {
        visited[i] = true;       // Marcar el nodo como visitado
        parent[i] = current;     // Establecer el nodo actual como padre
        toExplore.push_back(i);  // DFS es LIFO, entonces agregamos al final
